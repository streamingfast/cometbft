...
func (_m *Mempool) GetSenders(txKey types.TxKey) ([]p2p.ID, error) {
	ret := _m.Called(txKey)
	...
	var r0 []p2p.ID
	var r1 error
	if rf, ok := ret.Get(0).(func(types.TxKey) ([]p2p.ID, error)); ok {
		return rf(txKey)
	}
	if rf, ok := ret.Get(0).(func(types.TxKey) []p2p.ID); ok {
		r0 = rf(txKey)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]p2p.ID)
